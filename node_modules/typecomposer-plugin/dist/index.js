  ...
  function listRecursively(diretorio) {
    ...
    itens.forEach((item) => {
      ...
      if (statSync(pathFile).isDirectory())
        ...
        const file = relative(path4, pathFile);
...
${styleComponent.trim()}
  ...
  static async getStyleUrl(fileInfo, classInfo, styleUrl) {
    ...
    if (styleUrl == void 0)
      styleUrl = join2(dirname(fileInfo.path), `${classInfo.className}.scss`);
    if (styleUrl == void 0)
      return void 0;
    styleUrl = normalize(styleUrl);
    if (styleUrl && existsSync(styleUrl))
      return styleUrl;
    styleUrl = join2(dirname(fileInfo.path), `${classInfo.className}.css`);
    styleUrl = normalize(styleUrl);
    return existsSync(styleUrl) ? styleUrl : void 0;
  }
    ...
    closeBundle: {
      ...
      async handler() {
        if (indexjs && config) {
          ...
          let code = readFileSync2(file, "utf-8");
...
import { utimesSync, readdirSync as readdirSync4, statSync as statSync4, existsSync as existsSync3, readFileSync as readFileSync6 } from "node:fs";
...
var RegisterBuild = class _RegisterBuild {
  ...
  static async readExtends(classInfo) {
    if (classInfo.paranet != void 0 && classInfo.paranet.path != "") {
      try {
        const conteudo = readFileSync3(classInfo.paranet.path, "utf8");
...
import { dirname as dirname2, join as join3, normalize as normalize2, resolve } from "node:path";
var TemplateBuild = class {
  static getTemplateUrl(fileInfo, classInfo, templateUrl) {
    ...
    if (templateUrl == void 0)
      templateUrl = join3(dirname2(fileInfo.path), `${classInfo.className}.html`);
    if (templateUrl == void 0)
      return void 0;
    templateUrl = normalize2(templateUrl);
    if (templateUrl == void 0 || !existsSync2(templateUrl))
      return void 0;
    return templateUrl;
  }
  static async readHtml(fileInfo, classInfo, templateUrl) {
    if (templateUrl == void 0)
      return;
    try {
      let html = readFileSync4(templateUrl, "utf-8");
...
import { readFileSync as readFileSync5, readdirSync as readdirSync2, statSync as statSync2, writeFileSync as writeFileSync3 } from "node:fs";
...
((Router2) => {
  ...
  async function findRouterTsFiles(directory) {
    ...
    function searchRecursively(dir) {
      ...
      files.forEach((file) => {
        ...
        const isDirectory = statSync2(filePath).isDirectory();
        if (isDirectory) {
  ...
  async function watchChange(id, change, project) {
    if (change.event != "update" && (id.endsWith(".ts") || id.endsWith(".js")) && Router2.isRouterFile(basename(id))) {
...
import path2, { basename as basename2 } from "node:path";
...
((Theme2) => {
  ...
  async function findFiles(directory) {
    ...
    function searchRecursively(dir) {
      ...
      files2.forEach((file) => {
        ...
        const isDirectory = statSync3(filePath).isDirectory();
        if (isDirectory) {
          searchRecursively(filePath);
  ...
  async function watchChange(id, change, project) {
    if (change.event != "update" && (id.endsWith(".scss") || id.endsWith(".css")) && Theme2.isThemeFile(basename2(id))) {
      ...
      if (change.event == "delete") {
        ...
        if (fileTheme == void 0) {
          Theme2.putFileTheme(id, getFileName(basename2(id)));
        }
      }
    }
  }
...
// src/plugin/transpilator/ProjectBuild.ts
var ProjectBuild = class extends Project {
  constructor() {
    super();
  ...
  async load_node_modules_dependencys() {
    ...
    try {
      const itens = readdirSync4("node_modules");
      const pacotes = itens.filter((item) => statSync4(path3.join("node_modules", item)).isDirectory() && item !== "." && item !== "..").map((item) => path3.join("node_modules", path3.join(item, "package.json"))).filter((item) => existsSync3(item));
      for (const packageJsonPath of pacotes) {
        const packageJsonContent = readFileSync6(packageJsonPath, "utf-8");
  ...
  async isFileTemplate(filePath) {
    const fileInfos = [];
    const fileName = path3.basename(filePath).replace(path3.extname(filePath), "");
    const fileDirName = path3.dirname(filePath);
    for (const fileInfo of this.files.values()) {
      if (path3.dirname(fileInfo.path) == fileDirName) {
